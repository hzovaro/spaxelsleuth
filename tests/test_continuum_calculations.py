...
axs[0].imshow(d4000_map_new, vmin=1.1, vmax=3.5, cmap="afmhot", origin="lower")
axs[1].imshow(d4000_map_err_new, vmin=0, vmax=1, cmap="jet", origin="lower")
fig.suptitle(f"compute_d4000()")

# TEST: compute_continuum_intensity()
start_A = 4500
stop_A = 4700
cont_map, cont_map_std, cont_map_err = compute_continuum_intensity(data_cube_B, var_cube_B, lambda_vals_B_rest_A, start_A, stop_A, v_star_map)
fig, axs = plt.subplots(nrows=1, ncols=3)
axs[0].imshow(cont_map, vmin=0, vmax=0.1, cmap="afmhot", origin="lower")
axs[1].imshow(cont_map_std, vmin=0, vmax=0.01, cmap="jet", origin="lower")
axs[2].imshow(cont_map_err, vmin=0, vmax=0.01, cmap="jet", origin="lower")
fig.suptitle(f"compute_continuum_intensity()")

# TEST: compute_HALPHA_amplitude_to_noise()
AN_HALPHA_map = compute_HALPHA_amplitude_to_noise(data_cube_R, var_cube_R, lambda_vals_R_rest_A, v_star_map, v_map[0], dv=300)
fig, ax = plt.subplots()
ax.imshow(AN_HALPHA_map, vmin=0, vmax=500, cmap="afmhot", origin="lower")
fig.suptitle(f"compute_HALPHA_amplitude_to_noise()")

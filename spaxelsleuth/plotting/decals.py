...
###############################################################################
# Paths
decals_im_path = Path(settings["decals_im_path"])

###############################################################################
def get_decals_image(gal, ra_deg, dec_deg,
                   as_per_px=0.1, sz_px=500):
    """
    Download a DECaLS cutout image.
    ...
    # Download the image
    imname = decals_im_path / f"{gal}_{sz_px}px.jpg"
...
def plot_decals_image(df, gal, 
    ...
    """
    Download and plot the DECaLS image of a galaxy with RA and Dec in the supplied 
    pandas DataFrame. The images are stored in settings["decals_im_path"]. Note 
    that if the galaxy is outside the decals footprint, no image is plotted.
    ...
    # Load image
    if reload_image or (not os.path.exists(decals_im_path / f"{gal}_{sz_px}px.jpg")):
        # Download the image
        logger.warn(f"file {decals_im_path / f'{gal}_{sz_px}px.jpg'} not found. Retrieving image from decals...")
        if not get_decals_image(gal=gal, ra_deg=ra_deg, dec_deg=dec_deg,
                       as_per_px=as_per_px, sz_px=sz_px):
            return None
            
    im = mpimg.imread(decals_im_path / f"{gal}_{sz_px}px.jpg")

...
def load_HII_grid(drrecmode="0"):
    ...
    assert drrecmode in grid_df["drrecmode"].unique(
    ), f"drrecmode {drrecmode} not in HII region grid dataframe!"
    ...
    # Convert solar-scaled abundances to log(O/H) + 12
    grid_df["log(O/H) + 12"] = [
        zeta_to_logOH12[f"{zeta:.2f}"]
        for zeta in grid_df["Nebular abundance (Z/Zsun)"]
    ]
...
def load_pAGB_grid(drrecmode="0"):
    ...
    assert drrecmode in grid_df["drrecmode"].unique(
    ), f"drrecmode {drrecmode} not in pAGB grid dataframe!"
...
def load_AGN_grid(drrecmode="0"):
    ...
    grid_df.drrecmode = grid_df.drrecmode.astype(str)
    grid_df["Nebular abundance (Z/Zsun)"] = grid_df[
        "Nebular abundance (Z/Zsun)"].astype(float)
    assert drrecmode in grid_df["drrecmode"].unique(
    ), f"drrecmode {drrecmode} not in AGN dataframe!"
...
def load_shock_grid(model_type):
    assert model_type in ["total", "precursor", "shock"
                          ], "type must be one of total, precursor or shock!"

    # Load the dataframe
    grid_df = pd.read_csv(
        os.path.join(GRID_PATH, "MAPPINGS_shock_grid_13_fluxes.csv"))
    ...
    grid_df["Nebular abundance (Z/Zsun)"] = grid_df[
        "Nebular abundance (percentage solar)"] / 100.0
    ...
    if model_type != "total":
        grid_df = grid_df.drop(
            columns=[col for col in grid_df.columns if col.endswith("_tot")])
    if model_type != "precursor":
        grid_df = grid_df.drop(
            columns=[col for col in grid_df.columns if col.endswith("_pre")])
    if model_type != "shock":
        grid_df = grid_df.drop(
            columns=[col for col in grid_df.columns if col.endswith("_shc")])
    ...
    # Convert solar-scaled abundances to log(O/H) + 12
    grid_df["log(O/H) + 12"] = 8.76 + np.log10(
        grid_df["Nebular abundance (Z/Zsun)"])
...
def plot_BPT_with_grids(grid="HII",
                        ...
                        logOH12_shock=8.76,
                        drrecmode="0",
                        shock_model_type="total",
                        cmap_logU="cubehelix",
                        cmap_logOH12="cividis",
                        cmap_logEpeak="spring",
                        cmap_vel="plasma",
                        cmap_density="summer_r",
                        ls="-",
                        lw=1,
                        zorder=1,
                        colorbar=False):
    ...
    if grid == "HII":
        ...
        lines += [
            Line2D([0], [0],
                   color=cmap_logOH12(ii),
                   linewidth=logOH12_lw,
                   linestyle=ls) for ii in range(len(logOH12_vals))
        ]
        labels += [
            r"$\log({\rm O/H}) + 12 = %.2f$" % logOH12
            for logOH12 in logOH12_vals
        ]
        lines += [
            Line2D([0], [0],
                   color=cmap_logU(ii),
                   linewidth=logU_lw,
                   linestyle=ls) for ii in range(len(logU_vals))
        ]
        labels += [r"$\log(U) = %.1f$" % logU for logU in logU_vals]

        # label to indicate pressure
        axs[0].text(s=r"HII region models, $\log_{10}(P/k) = %.2f$" % logPk,
                    x=0.1,
                    y=1.05,
        ...
        for ii, xratio in enumerate(["log N2", "log S2", "log O1"]):
            ...
            for logU_idx, logU in enumerate(logU_vals):
                ...
                axs[ii].plot(df1.loc[:, xratio].values,
                             df1.loc[:, "log O3"].values,
                             ls=ls,
                             c=cmap_logU(logU_idx),
                             zorder=zorder,
                             linewidth=logU_lw)
        ...
        for ax in axs:
            ...
            ax.set_position([
                box_ax.x0, box_ax.y0 + 0.25 * box_ax.height, box_ax.width,
                box_ax.height * 0.66
            ])
        axs[1].legend(lines,
                      labels,
    ...
    if grid == "pAGB":
        ...
        assert len(grid_df["age (yr)"].unique()
                   ) == 1, "pAGB grid contains multiple ages!"
        assert len(grid_df["Stellar abundance (log(Z/Zsun))"].unique()
                   ) == 1, "pAGB grid contains multiple stellar abundances!"
        ...
        axs[0].text(
            s=r"pAGB models, $\log_{10}(P/k) = %.2f,\, t = %.2f\,{\rm Gyr},\,\log[Z/Z_\odot] = %.2f$"
            % (logPk, age_gyr, logZ_star),
            x=0.1,
            y=1.05,
            transform=axs[0].transAxes)
    ...
    if grid == "AGN":
        ...
        assert len(
            grid_df["Gamma"].unique()) == 1, "AGN grid contains Gamma values!"
        assert len(
            grid_df["p_NT"].unique()) == 1, "AGN grid contains p_NT values!"
        ...
        lines += [
            Line2D([0], [0],
                   color=cmap_logEpeak(ii),
                   linewidth=logEpeak_lw,
                   linestyle=ls) for ii in range(len(logEpeak_vals))
        ]
        labels += [
            r"$\log(E_{\rm peak} \, [{\rm keV}]) = %.2f$" % logEpeak
            for logEpeak in logEpeak_vals
        ]
        lines += [
            Line2D([0], [0],
                   color=cmap_logU(ii),
        ...
        # label to fixed model parameters
        axs[0].text(
            s=r"AGN models, $\log({\rm O/H}) + 12 = %.2f,\,\log_{10}(P/k) = %.2f,\, \Gamma = %.2f,\,p_{\rm NT} = %.2f$"
            % (logOH12_AGN, logPk, Gamma, p_NT),
            x=0.1,
            y=1.05,
            transform=axs[0].transAxes)
    ...
    if grid == "shock":
        ...
        assert len(grid_df["log_10(P_mag/P_gas)"].unique()
                   ) == 1, "Shock grid contains multiple magnetic parameters!"
        ...
        lines += [
            Line2D([0], [0],
                   color=cmap_density(ii),
                   linewidth=density_lw,
                   linestyle=ls) for ii in range(len(density_vals))
        ]
        labels += [
            r"$n = %.0f\,\rm cm^{-3}$" % density for density in density_vals
        ]
        lines += [
            Line2D([0], [0],
                   color=cmap_vel(ii),
                   linewidth=vel_lw,
                   linestyle=ls) for ii in range(len(vel_vals))
        ]
        labels += [
            r"$v_{\rm shock} = %.0f\,\rm km\,s^{-1}$" % vel for vel in vel_vals
        ]

        # label to fixed model parameters
        axs[0].text(
            s=r"Shock models (%s), $\log_{10}(P/k) = %.2f,\, \log_{10}(P_{\rm mag}/P_{\rm gas}) = %.2f$"
            % (shock_model_type, logOH12_shock, logAlpha0),
            x=0.1,
            y=1.05,
            transform=axs[0].transAxes)
...
def plot_grids(col_x,
    ...
    if grid == "HII":
        ...
        for logU_idx, logU in enumerate(logU_vals):
            ...
            cond &= grid_df["log(U) (inner)"] == logU
            df1 = grid_df[cond].sort_values("log(O/H) + 12")
            ax.plot(df1.loc[:, col_x].values,
                    df1.loc[:, col_y].values,
                    ls=ls,
                    c=cmap_logU(logU_idx),
                    zorder=zorder,
                    linewidth=logU_lw,
    ...
    if shrink_axis:
        box_ax = ax.get_position()
        ax.set_position(
            [box_ax.x0, box_ax.y0, box_ax.width * .75, box_ax.height])
    if legend:
        ax.legend(lines,
                  labels,
                  loc='center left',

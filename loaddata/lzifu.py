...
#######################################################################
def load_lzifu_galaxies(ncomponents, bin_type,
                        eline_SNR_min,
                        gal=None,
    ...
    """
    load_lzifu_galaxies(): load a DataFrame containing emission line fluxes,
    etc. for a galaxy (or galaxies) re-processed using LZIFU.

    If gal is specified, then the DataFrame for that particular galaxy is 
    returned. Otherwise, a DataFrame containing ALL galaxies in the high-S/N 
    subset of LZIFU galaxies (as defined in make_dfs_lzifu.py) is returned.
    ...
    if gal is None:
        ...
        df_fname = f"lzifu_subsample.hd5"
    else:
...
if __name__ == "__main__":
    ...
    df_nocut = load_lzifu_galaxies(gal=gal,
                            ncomponents="recom", bin_type="default",
